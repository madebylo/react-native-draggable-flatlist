...
import { Dimensions } from "react-native"; // Reanimated primitives

import { scrollTo, useAnimatedReaction, useDerivedValue, useSharedValue } from "react-native-reanimated"; // Project constants & helpers

import { DEFAULT_PROPS, SCROLL_POSITION_TOLERANCE, isWeb } from "../constants"; // Custom context hooks
 ...
 */
var OuterScrollState;
/**
 * Simple direction helper for comparisons
 */

(function (OuterScrollState) {
  OuterScrollState[OuterScrollState["Idle"] = 0] = "Idle";
  OuterScrollState[OuterScrollState["Up"] = 1] = "Up";
  OuterScrollState[OuterScrollState["Down"] = 2] = "Down";
...
(function (ScrollDir) {
  ScrollDir[ScrollDir["None"] = 0] = "None";
  ScrollDir[ScrollDir["Up"] = -1] = "Up";
  ScrollDir[ScrollDir["Down"] = 1] = "Down";
})(ScrollDir || (ScrollDir = {}));

export function useAutoScroll(outerScrollRef, outerScrollOffset, pointerY, activeKey, pointerType, innerAnimatedRef) {
  /* ------------------------------------------------------------------------ */
  ...
  const {
    flatlistRef
  } = useRefs(); // Component‑level props (with sensible fallbacks)

  const {
    autoscrollThreshold = DEFAULT_PROPS.autoscrollThreshold,
    autoscrollSpeed = DEFAULT_PROPS.autoscrollSpeed
  } = useProps(); // Continuously updated animated values provided by <DraggableFlatList />

  const {
    scrollOffset,
    // current y‑scroll offset of the FlatList
    scrollViewSize,
    // content height of the FlatList
    containerSize,
    // height of the visible viewport
    activeCellSize,
    // height of the row that is being dragged
    hoverOffset,
    // y‑coordinate of the hover component in list coords
  ...
  const hoverScreenOffset = useDerivedValue(() => hoverOffset.value - scrollOffset.value); // Has the list already reached its very top / bottom?

  const isScrolledUp = useDerivedValue(() => scrollOffset.value - SCROLL_POSITION_TOLERANCE <= 0);
  const isScrolledDown = useDerivedValue(() => scrollOffset.value + containerSize.value + SCROLL_POSITION_TOLERANCE >= scrollViewSize.value); // Distance from hover component to each edge of the viewport
  ...
  const isAtTopEdge = useDerivedValue(() => distToTopEdge.value <= autoscrollThreshold);
  const isAtBottomEdge = useDerivedValue(() => distToBottomEdge.value <= autoscrollThreshold); // Mutable shared value that we keep chasing via scrollTo

  const scrollTarget = useSharedValue(0); // Is a drag currently happening?

  const dragActive = useDerivedValue(() => activeIndexAnim.value >= 0); // Whenever a drag starts, reset scrollTarget to the current offset

  useAnimatedReaction(() => dragActive.value, (cur, prev) => {
  ...
  const shouldScrollInner = useDerivedValue(() => {
    ...
    const blocked = isAtTopEdge.value && isScrolledUp.value || isAtBottomEdge.value && isScrolledDown.value;
  ...
  useDerivedValue(() => {
    ...
    if (!innerAnimatedRef) return; // How deep into the threshold did we move? (0‑1)

    const dist = isAtTopEdge.value ? distToTopEdge.value : distToBottomEdge.value;
    const pct = 1 - dist / autoscrollThreshold; // Map that percentage to a delta value

    const delta = pct * autoscrollSpeed; // Calculate the next y‑offset based on direction

    const next = isAtTopEdge.value ? Math.max(0, scrollOffset.value - delta) : Math.min(scrollOffset.value + delta, scrollViewSize.value - containerSize.value); // Persist & apply the scroll
  ...
  if (!outerScrollRef || !outerScrollOffset) return; // Shared values to keep track of state & geometry

  const outerState = useSharedValue(OuterScrollState.Idle);
  const outerTopY = useSharedValue(0);
  const outerBottomY = useSharedValue(0);
  const attemptedOff = useSharedValue(0);
  const lastDir = useSharedValue(ScrollDir.None);
  /* ---------------------- 1) Temporarily disable parent scroll ------------ */

  useEffect(() => {
    const node = outerScrollRef === null || outerScrollRef === void 0 ? void 0 : outerScrollRef.current;
    if (!node || (pointerType === null || pointerType === void 0 ? void 0 : pointerType.value) === 2) return; // keep mouse wheel enabled
    ...
    const setScrollEnabled = enabled => {
      if (typeof node.setNativeProps === "function") {
        ...
        const el = node; //@ts-ignore

        el.style.overflowY = enabled ? "auto" : "hidden"; //@ts-ignore
    ...
    setScrollEnabled(!locked); // Cleanup – always re‑enable scrolling on unmount / key change
  ...
  useEffect(() => {
    if (!(outerScrollRef !== null && outerScrollRef !== void 0 && outerScrollRef.current)) return;

    const measure = () => {
      var _outerScrollRef$curre;

      // @ts-ignore
      (_outerScrollRef$curre = outerScrollRef.current) === null || _outerScrollRef$curre === void 0 ? void 0 : _outerScrollRef$curre.measureInWindow((_, y, __, h) => {
        outerTopY.value = y;
    ...
    measure(); // React‑Native: listen to orientation / window changes

    const dimSub = Dimensions.addEventListener("change", measure); // Web: ResizeObserver for layout shifts

    let ro; // @ts-ignore

    if (isWeb && typeof window !== "undefined" && window.ResizeObserver) {
      const el = outerScrollRef.current; //@ts-ignore – Type narrowing for older RN types

      const ROs = window.ResizeObserver;
      ro = new ROs(measure);
      ro.observe(el);
    }
  ...
  useDerivedValue(() => {
    const y = pointerY === null || pointerY === void 0 ? void 0 : pointerY.value;
  ...
  useDerivedValue(() => {
    if (outerState.value === OuterScrollState.Idle || activeKey == null || (pointerY === null || pointerY === void 0 ? void 0 : pointerY.value) == null) return; // Distance from pointer to triggering edge

    const dist = outerState.value === OuterScrollState.Up ? pointerY.value - outerTopY.value : outerBottomY.value - pointerY.value;
    const pct = 1 - dist / autoscrollThreshold;
    if (pct <= 0) return;
    const dir = outerState.value === OuterScrollState.Up ? ScrollDir.Up : ScrollDir.Down;
  ...
  useAnimatedReaction(() => outerScrollOffset.value, (cur, prev) => {
    if (outerState.value === OuterScrollState.Idle || prev == null) return;
    const moved = lastDir.value === ScrollDir.Down ? cur > prev : cur < prev;
